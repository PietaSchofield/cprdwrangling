...
```{r the-db_R, comment=NA,echo=F}
if(knitr::is_html_output()){
  rtrhd::list_db(dbname) |> plibr::display_data(buttons=F)
}else{
  rtrhd::list_db(dbname) |> knitr::kable(format="markdown")
}
```
</div>
...
<div style="font-size: 0.5em;">
```{r ta-dah_2_R, comment=NA, echo=F}
if(knitr::is_html_output()){
  dementia_trends |> head() |> mutate(patid="********") |> plibr::display_data(buttons=F)
}else{
  dementia_trends |> head() |> mutate(patid="********") |> knitr::kable()
}
```
</div>
...
```{r plotit_R, comment=NA, echo=F, out.width="100%",warning=F,message=F}
print(dementia_trends_plot)
```

```{r show-plot-markdown, echo=FALSE, eval=!knitr::is_html_output(), results='asis'}
...
```{r show-plot-2-markdown, echo=FALSE, eval=!knitr::is_html_output(), results='asis'}
figpath <- "https://github.com/PietaSchofield/cprdwrangling/tree/main/README_files/figure-gfm"
figfile <- file.path(figpath,"visualise_2_R.png")
cat(paste0("\n\n![Dementia trends plot](",figfile,")\n\n"))
```
